...
test('cleanup command order removes worktree before pruning and deleting branch', async () => {
  const source = await readFile('scripts/agent-bridge/cleanup-adapter.mjs', 'utf8');
...
test('remote cleanup is exact, non-force, and follows local cleanup', async () => {
  const source = await readFile('scripts/agent-bridge/cleanup-adapter.mjs', 'utf8');
  assert.match(source, /ls-remote', '--heads', 'origin', exactRef/);
  assert.doesNotMatch(source, /push.*--force/);
  assert.ok(source.indexOf("['branch', '-d'") < source.indexOf("['push', 'origin', '--delete'"));
});
...
function fake(overrides = {}) { const events = []; const ops = { verifyMergedPullRequest: async () => { events.push('verify'); return { merged: true, prNumber: 1, headBranch: 'agent/36-cleanup' }; }, isWorktreeClean: async () => { events.push('clean'); return true; }, syncMain: async () => events.push('sync'), removeWorktree: async () => events.push('remove'), pruneWorktrees: async () => events.push('prune'), isLocalBranchAttached: async () => { events.push('attached'); return false; }, deleteLocalBranch: async () => events.push('local'), remoteBranchExists: async () => { events.push('exists'); return true; }, deleteRemoteBranch: async () => events.push('remote'), verifyRemoteBranchAbsent: async () => events.push('absent'), ...overrides }; return { events, ops }; }
