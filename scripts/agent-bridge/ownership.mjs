...
const archiveDir = (root) => path.join(runtimeDir(root), 'child-history');
...
export function validateChild(record, repositoryIdentity) {
  ...
  if (!Number.isSafeInteger(record.childPid) || record.childPid < 1 || typeof record.childStartIdentity !== 'string' || !record.childStartIdentity || typeof record.childPlatform !== 'string' || !validIso(record.childObservedAt || record.spawnedAt)) throw new Error('malformed child process identity');
  if (!Number.isSafeInteger(record.sequence) || record.sequence < 1 || !validIso(record.spawnedAt) || !validIso(record.lastSeenAt) || !['running'].includes(record.state)) throw new Error('malformed child registry state');
  return record;
}
...
export async function acquireOwnership(root, { mode, runId, now = () => Date.now(), ownerPid = process.pid, ownerStartIdentity, ownerPlatform, ownerExecutableIdentity, ownerObservedAt, processIdentity = getProcessIdentity, leaseMs = DEFAULT_LEASE_MS, tokenGenerator = () => randomBytes(32).toString('hex'), fs = { mkdir, open, readFile, rename, rm }, processInspector } = {}) {
  ...
  const heartbeat = async (time = now()) => { const current = await assertToken(); if (!identityWasProvided) { const observed = await processInspector(current.ownerPid, current.ownerStartIdentity); if (!observed.exists || observed.identityMatch !== true) throw new Error(observed.exists ? 'owner process identity mismatch' : 'owner process is absent'); } const at = new Date(time).toISOString(); await atomicWrite(target, { ...current, heartbeatAt: at, leaseExpiresAt: new Date(time + leaseMs).toISOString(), ownerObservedAt: at }, fs); };
...
export async function registerChild(root, owner, { batchId, runId, sequence, issueNumber, childPid, childStartIdentity, childPlatform = process.platform, childExecutableIdentity = null, childObservedAt = new Date().toISOString(), now = () => Date.now(), fs = { mkdir, open, rename, rm } } = {}) {
  ...
  const record = { schemaVersion: CHILD_SCHEMA_VERSION, repositoryIdentity: repositoryIdentity(root), ownerTokenFingerprint: capabilityFingerprint(owner.ownershipToken), runId, batchId, sequence, issueNumber: Number.isSafeInteger(issueNumber) ? issueNumber : null, childPid, childStartIdentity, childPlatform, childExecutableIdentity, childObservedAt, spawnedAt: new Date(now()).toISOString(), lastSeenAt: new Date(now()).toISOString(), state: 'running' };
...
export async function updateChild(root, owner, runId, patch, fs = { readFile, mkdir, open, rename, rm }) { const target = path.join(childDir(root), `${runId}.json`); const record = validateChild(JSON.parse(await fs.readFile(target, 'utf8')), repositoryIdentity(root)); if (record.ownerTokenFingerprint !== capabilityFingerprint(owner?.ownershipToken || '')) throw new Error('execution ownership token mismatch'); const updated = { ...record, ...patch, lastSeenAt: patch.lastSeenAt || new Date().toISOString() }; validateChild(updated, repositoryIdentity(root)); await atomicWrite(target, updated, fs); return updated; }
export async function archiveAndRemoveChild(root, owner, runId, terminal, fs = { readFile, mkdir, open, rename, rm }) { const target = path.join(childDir(root), `${runId}.json`); const record = validateChild(JSON.parse(await fs.readFile(target, 'utf8')), repositoryIdentity(root)); if (record.ownerTokenFingerprint !== capabilityFingerprint(owner?.ownershipToken || '')) throw new Error('execution ownership token mismatch'); const archived = { ...record, terminalState: terminal.state, exitCode: Number.isSafeInteger(terminal.exitCode) ? terminal.exitCode : null, exitedAt: terminal.exitedAt || new Date().toISOString(), failureCategory: terminal.failureCategory || null }; await atomicWrite(path.join(archiveDir(root), `${runId}.json`), archived, fs); await fs.rm(target, { force: false }); return archived; }
...
export { capabilityFingerprint, ownerPath, childDir, archiveDir };
