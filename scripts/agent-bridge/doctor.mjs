...
export function protectionChecks(p) {
  ...
  return [
    ['main-protection', !!p],
    ['pull-request-required', !!p?.required_pull_request_reviews],
...
export async function doctor(adapter = { protection: () => run('gh', ['api', `repos/NastyBaster/online-market/branches/${expected.branch}/protection`]).then(JSON.parse) }) {
  const checks = [];
  for (const [name, cmd, args] of [['git', 'git', ['--version']], ['gh-auth', 'gh', ['auth', 'status']], ['codex', 'codex', ['exec', '--help']]]) { try { await run(cmd, args); checks.push({ name, pass: true }); } catch (e) { checks.push({ name, pass: false, detail: e.message }); } }
